 ...
 * and intrinsic dimensions (used by next/image to reserve layout space and
 * avoid CLS).
 *
 * Swapping placeholders for real photos:
 *   1. Drop the real file into /public/images using the same filename, OR
 *   2. Point `src` at a CDN URL (and add the host to next.config.mjs
 *      `images.remotePatterns`).
 * Update `alt` to describe the real photo for accessibility + SEO.
 *
 * The current placeholders are generated by `scripts/generate-placeholders.mjs`
